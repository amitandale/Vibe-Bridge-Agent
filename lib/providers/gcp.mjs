// lib/providers/gcp.mjs
// Test-safe sleep: do NOT unref(). The node:test runner may exit early otherwise.
...
export async function deploy({ repo, framework, fetchImpl = globalThis.fetch }){
  const res = await fetchImpl('https://gcp.example/deploy', { method:'POST', body: JSON.stringify({ repo, framework }) });
  const body = await res.json().catch(() => ({}));
  if (!res.ok) throw mapError(res.status, body);
  return { id: body.id || 'deploy_gcp_123' };
}

export async function status({ id, fetchImpl = globalThis.fetch, maxAttempts = 5, backoffMs = 50 }){
  for (let attempt = 1; attempt <= maxAttempts; attempt++){
    try {
      const res = await fetchImpl(`https://gcp.example/deploy/${id}/status`);
      const body = await res.json().catch(() => ({}));
...
export async function previewUrl({ id, fetchImpl = globalThis.fetch }){
  const res = await fetchImpl(`https://gcp.example/deploy/${id}/preview`);
  const body = await res.json().catch(() => ({}));
  if (!res.ok) throw mapError(res.status, body);
  return { url: body.url || 'https://preview.example.gcp.run' };
}
