...
export function resolveLayout({ projectId, lane, user='devops' }){
  const root = `/opt/github-runner/${projectId}/${lane}`;
...
export function renderSystemdUnit({ projectId, lane, url, token, owner, repo, user='devops' }){
  const { root, work } = resolveLayout({ projectId, lane, user });
...
export function planProvision({ projectId, lane, owner, repo, token, user='devops' }){
  if (!projectId || !lane) throw new Error('MISSING_PROJECT_OR_LANE');
  if (!owner || !repo) throw new Error('PROJECT_NOT_BOUND');
  if (!token) throw new Error('MISSING_TOKEN');
  const unit = renderSystemdUnit({ projectId, lane, owner, repo, token, user });
  const text = serializeUnit(unit);
  const layout = resolveLayout({ projectId, lane, user });
  return {
    labels: unit.labels,
    files: [
      { path: `/etc/systemd/system/${layout.svcName}`, mode: '0644', content: text }
    ],
    layout
  };
}
