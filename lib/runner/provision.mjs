...
export function renderSystemdUnit({ projectId, lane, owner, repo, token, user='devops', rootBase='/opt/github-runner', systemdDir='/etc/systemd/system' }){
  const { root, work, svcName } = resolveLayout({ projectId, lane, user, rootBase });
  ...
  const unitPath = `${systemdDir}/${svcName}`;
  return {
    unitPath,
    Unit: {
      Description: `GitHub Runner (${projectId}:${lane})`,
      After: 'network.target'
    },
    Service: {
      User: user,
      WorkingDirectory: root,
      ExecStartPre: register.map(a => /\s/.test(a) ? `'${a.replace(/'/g, "'\''")}'` : a).join(' '),
...
export function planProvision({ projectId, lane, owner, repo, token, user='devops', rootBase='/opt/github-runner', systemdDir='/etc/systemd/system' }){
  ...
  const unit = renderSystemdUnit({ projectId, lane, owner, repo, token, user, rootBase, systemdDir });
  const text = serializeUnit(unit);
  const layout = resolveLayout({ projectId, lane, user, rootBase });
  return {
    labels: unit.labels,
    files: [
      { path: unit.unitPath, mode: '0644', content: text }
    ],
    layout
  };
}
...
// --- BA-20b helpers (non-invasive) ---
export function renderRunnerEnv({ name, labels, scope, owner, repo }){
  const lines = [
    `RUNNER_NAME=${name}`,
    `RUNNER_LABELS=${labels}`,
    scope === 'org' ? `RUNNER_SCOPE=org` : `RUNNER_SCOPE=repo`,
    owner ? `RUNNER_OWNER=${owner}` : '',
    repo ? `RUNNER_REPO=${repo}` : '',
  ].filter(Boolean);
  return lines.join('\n') + '\n';
}

export async function detectRunnerVersion({ fsAdapter, binDir }){
  // Try to read .runner or package metadata under bin/. Fallback to 'unknown'.
  try {
    const txt = await fsAdapter.readFile(`${binDir}/.runner-version`, 'utf8');
    return txt.trim();
  } catch {
    return 'unknown';
  }
