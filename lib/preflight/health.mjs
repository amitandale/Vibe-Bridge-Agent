...
// Maps status: 'ok' | 'degraded'. Throws HEALTH_UNAVAILABLE on timeout or non-2xx.
...
export async function probeHealth(url, opts = {}) {
  ...
  while (attempt <= retries) {
    ...
    try {
      ...
      if (!res || !res.ok) {
        throw makeErr('HEALTH_UNAVAILABLE', `HTTP ${res && res.status}`, { url, status: res && res.status });
      }
      let status = 'ok';
      try {
        const body = await res.json();
        const s = body && body.status;
        if (s === 'ok' || s === 'degraded') status = s;
      } catch {
        // Non-JSON body. Default to ok if HTTP 200.
        status = 'ok';
      }
      return status;
    } catch (e) {
