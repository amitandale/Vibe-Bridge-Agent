...
function deterministicIsoTimestamp({ labels=[], diff='', fileContents={}, failingTests=[] } = {}) {
  try {
    ...
    const payload = JSON.stringify({ labels, diff, keys, failingTests });
...
function enforceGlobalBudget({ sections, budgets }) {
  const priority = ['extras','linked_tests','templates','spec_canvas','contracts']; // never drop diff_slices
...
export function planFromSignals({ labels=[], diff='', fileContents = {}, failingTests=[], templatesRegistry } = {}) {
  ...
  for (const r of renamed) {
    provenance.push({ source:'git', generator:'ba/planner', created_at: createdAt, old_path:r.from || undefined, new_path:r.to || undefined });
  }
