...
// Re-entrancy guard to stop recursive migrate->open->migrate loops.
// This prevents runaway recursion if client.mjs calls migrate() during open().
const MIGRATING_FLAG = '__VIBE_MIGRATING__';

export function migrate({ sqlDir, db: providedDb } = {}){
  if (globalThis[MIGRATING_FLAG]){
    // Skip nested invocation and report a safe no-op.
    return { ok: true, applied: [], skipped: true };
  }
  globalThis[MIGRATING_FLAG] = true;
  try {
    const db = providedDb || open();
    ...
    globalThis[MIGRATING_FLAG] = false;
  }
}
