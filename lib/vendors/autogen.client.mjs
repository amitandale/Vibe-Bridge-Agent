...
function hmacSha256Hex(key, bodyStr) {
  // Treat key as utf8 by default. If hex-looking, use it as hex bytes.
  let keyBuf;
  if (/^[0-9a-fA-F]{64}$/.test(key || '')) {
    keyBuf = Buffer.from(key, 'hex');
  } else if (/^[A-Za-z0-9+/=]+$/.test(key || '') && (key || '').length % 4 === 0) {
    try { keyBuf = Buffer.from(key, 'base64'); } catch { keyBuf = Buffer.from(String(key ?? ''), 'utf8'); }
  } else {
    keyBuf = Buffer.from(String(key ?? ''), 'utf8');
  }
...
async function doRequest(url, bodyStr, timeoutMs, retries) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (http && typeof http.fetch === 'function') {
        ...
        const r = await fetchWithTimeout(url, { method: 'POST', headers, body: bodyStr }, timeoutMs);
        if (r.timedOut) throw makeError('UPSTREAM_UNAVAILABLE', 'autogen timeout', {}, 0);
        res = r.res;
      }
...
async function runAgents({ teamConfig = {}, messages = [], contextRefs = [], idempotencyKey = '' } = {}) {
  ...
  const url = `${baseUrl.replace(/\/+$/, '')}/runAgents`;

  const json = await doRequest(url, bodyStr, timeoutMs, retries);
