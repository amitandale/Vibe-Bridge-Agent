...
function mapStatusToCode(Codes, status, transportErrName) {
  if (transportErrName === 'AbortError') return 'UPSTREAM_UNAVAILABLE';
  if (!status) return 'UPSTREAM_UNAVAILABLE';
  if (status === 400) return Codes?.BAD_REQUEST ?? 'BAD_REQUEST';
  if (status === 401) return Codes?.UNAUTHENTICATED ?? 'UNAUTHENTICATED';
  if (status === 403) return Codes?.FORBIDDEN ?? 'FORBIDDEN';
  if (status === 404) return Codes?.NOT_FOUND ?? 'NOT_FOUND';
  if (status === 429) return Codes?.RATE_LIMITED ?? 'RATE_LIMITED';
  if (status >= 500) return Codes?.UPSTREAM_UNAVAILABLE ?? 'UPSTREAM_UNAVAILABLE';
