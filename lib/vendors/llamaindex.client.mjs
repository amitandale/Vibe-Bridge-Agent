...
export function makeLlamaIndexClient(opts = {}) {
  ...
  // Normalize possible shapes: { docIds: [...] } or { ids: [...] } or { documents: [...] }
  const docIds =
    (Array.isArray(raw?.docIds) && raw.docIds) ||
    (Array.isArray(raw?.doc_ids) && raw.doc_ids) ||
    (Array.isArray(raw?.ids) && raw.ids) ||
    (Array.isArray(raw?.documents) && raw.documents.map(d => d.id ?? d.doc_id ?? d.node_id).filter(Boolean)) ||
    [];
  return { ok: !!res?.ok, status: res?.status ?? 0, docIds };
}

  async function query({ projectId, q, topK } = {}) {
    const resolvedTopK = toInt(topK ?? process.env.LI_TOP_K, 5);
    const payload = { projectId, q, k: resolvedTopK, top_k: resolvedTopK };
    ...
    for (const r of source) {
      ...
      const node_id = r.node_id ?? id ?? undefined;
    ...
    // Provide multiple shapes for compatibility
    const nodes = items.map(({ text, score, id, node_id, metadata }) => ({
      text, ...(score !== undefined ? { score } : {}),
      ...(id !== undefined ? { id } : {}),
      ...(node_id !== undefined ? { node_id } : {}),
      ...(metadata !== undefined ? { metadata } : {}),
    }));
    const chunks = nodes.map(x => ({ ...x }));
    const results = nodes.map(x => ({ ...x }));

    return { items, nodes, chunks, results, raw, top_k: resolvedTopK };
  }
