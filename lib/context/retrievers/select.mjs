...
\1
  // Prefer explicit remote when requested
  if (prefer === 'llamaindex-remote') return 'llamaindex-remote';
  if ((env.BA_RETRIEVER || '').toLowerCase() === 'llamaindex-remote') return 'llamaindex-remote';

  // If LLAMAINDEX_URL is configured, prefer remote client by default
  if (env.LLAMAINDEX_URL && String(env.LLAMAINDEX_URL).length > 0) return 'llamaindex-remote';
