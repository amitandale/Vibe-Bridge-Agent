...
export async function checkFs({ projectId }, adapter, opts={}){
  ...
  const bavail = typeof vfs.bavail === 'bigint' ? vfs.bavail : BigInt(vfs.bavail);
  const frsize = typeof vfs.frsize === 'bigint' ? vfs.frsize : BigInt(vfs.frsize);
  const bytes = bavail * frsize; // BigInt
  const availGiB = Number(bytes) / 1073741824; // 1024**3
