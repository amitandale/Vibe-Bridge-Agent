...
/** Resolve template path. Priority: explicit file > dir/docker-compose.yml > package assets. */
export async function resolveTemplate({ templateFile, templateDir } = {}){
  if (templateFile) {
    try { await stat(templateFile); return templateFile; } catch {}
  }
  if (templateDir) {
...
export async function renderWorkspace({
  ...
  // Fail closed if template missing
  ...
  for (const lane of lanes){
    ...
    // docker-compose.yml — do simple placeholder substitution, but keep template valid if no placeholders
    let body = tpl
