...
function compressText(it, maxPerFileTokens, estimateTokens){
  ...
  const perTokChars = 4; // estimator heuristic
  const maxChars = Math.max(1, Math.floor(maxPerFileTokens * perTokChars));
  const head = original.slice(0, Math.floor(maxChars/2));
  const tail = original.slice(-Math.ceil(maxChars/2));
  const stitched = (it.symbol ? `// ${it.symbol}\n` : '') + head + '\n...\n' + tail;
  const t = estimateTokens({ text: stitched }).inputTokens;
  return { text: stitched, tokens: t, compressed: true };
}
...
function dedupWithinSection(items){
  ...
  for (const it of items){
    const key = (it.path || it.id || '') + '|' + (it.loc?.start_line || 0) + '|' + (it.symbol || '') + '|' + textOf(it);
...
export async function assemble(pack, { model='default' } = {}){
  ...
  for (const name of ORDER){
    ...
    for (const s of nonDroppable){
      ...
      const secOver   = secCapFiles ? nextSecFiles > secCapFiles : false;
