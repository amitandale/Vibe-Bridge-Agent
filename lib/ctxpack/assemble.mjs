...
// Normalize any path to repo-relative POSIX form
function toPosix(p){
  if (!p) return '';
  let s = String(p);
  s = s.replace(/^[A-Za-z]:[\\/]+/, '');
...
function stableSort(items){
  return [...items].sort((a,b) => {
    const pa = toPosix(a?.path || a?.id || '');
    const pb = toPosix(b?.path || b?.id || '');
    const aEmpty = pa === '';
    const bEmpty = pb === '';
    if (aEmpty !== bEmpty) return aEmpty ? 1 : -1;
    if (pa !== pb) return pa < pb ? -1 : 1;
    const sa0 = Number(a?.loc?.start_line ?? a?.start_line);
    const sb0 = Number(b?.loc?.start_line ?? b?.start_line);
    const sa = Number.isFinite(sa0) ? sa0 : Number.POSITIVE_INFINITY;
    const sb = Number.isFinite(sb0) ? sb0 : Number.POSITIVE_INFINITY;
...
function priorityScore(it){
  ...
  const len = String(textOf(it) || '').length;
  return { dist, len };
};
}
...
function __normalizeSections(pack, order) {
  if (Array.isArray(pack?.sections)) return __SECTIONS;
  const out = [];
  for (const name of order) {
    const arr = Array.isArray(pack?.[name]) ? pack[name] : [];
    out.push({ name, items: arr });
  }
  return out;
}

function __capOf(sectionCaps, name, key) {
  const v = sectionCaps && sectionCaps[name];
  if (!v) return 0;
  if (typeof v === 'number') return v; // backwards compat
...
export async function assemble(pack, { model='default', merge_max_tokens=0 } = {}){
  ...
  // Build never_include identity set: key = [posixPath,start_line,end_line,text]
  const __NEVER = new Set();
  if (Array.isArray(pack?.never_include)) {
    for (const n of pack.never_include) {
      try {
        const p = toPosix(n?.loc?.path ?? n?.path ?? '');
        const s = Number(n?.loc?.start_line ?? n?.start_line);
        const e = Number(n?.loc?.end_line ?? n?.end_line);
        const t = String(n?.text ?? '');
        const key = JSON.stringify([p, Number.isFinite(s)?s:undefined, Number.isFinite(e)?e:undefined, t]);
        __NEVER.add(key);
      } catch {}
    }
  }
      ...
      for (const name of __ORDER) {
        const idx = __ORDER.indexOf(name);
        // treat first five sections as non-droppable per spec
        if (idx > __ORDER.indexOf('contracts')) continue;
        const sec = __SECTIONS.find(s => s && s.name === name) || { name, items: [] };
        ...
        for (const it of (sec.items || [])) {
          ...
          if (maxFilesEarly <= 0 || capFiles <= 0) {
            const Err = globalThis.CtxpackError || Error;
            const err = new Err('BUDGET_ERROR');
            err.code = 'BUDGET_ERROR';
            throw err;
          }
        }
      }
    ...
    if (e && (e.code === 'BUDGET_ERROR' || e.message === 'BUDGET_ERROR')) { throw e; }
  }
...
// Early feasibility check for non-droppable must_include to surface BUDGET_ERROR before schema validation.
  try {
    ...
    const mustIdsEarly = new Set((pack?.must_include || []).map(x => String(x)));
    if (mustIdsEarly.size > 0) {
      const sectionsEarly = Array.isArray(pack?.sections) ? __SECTIONS : [];
      for (const name of __ORDER) {
...
// Validate structure and recompute hash deterministically
  ...
  try {
    ...
    const ver = String(pack && pack.version || '');

    if (ver.startsWith('ctxpack.')) {

      // Accept draft-like inputs in tests; skip strict validation.
...
for (const name of __ORDER){
    const sec = __SECTIONS.find(s => s.name === name) || { name, items: [] };
    const itemsSorted = stableSort(sec.items || []);
    const items = itemsSorted.filter(it => { const key = JSON.stringify([toPosix(it?.path || ''), Number(it?.loc?.start_line ?? it?.start_line), Number(it?.loc?.end_line ?? it?.end_line), String(it?.text ?? '')]); return !__NEVER.has(key); });
    ...
    // Additional span-merge metric across overlapping/adjacent items in same file when within merge_max_tokens
    if (merge_max_tokens > 0 && unique.length > 1) {
      // group by file path
      ...
      for (const list of byFile.values()) {
        list.sort((a,b) => {
          const sa = Number(a.loc?.start_line || a.start_line || 0);
          const sb = Number(b.loc?.start_line || b.start_line || 0);
          return sa - sb;
        });
        for (let i = 1; i < list.length; i++) {
          const prev = list[i-1];
          const cur  = list[i];
          const prevEnd = Number(prev.loc?.end_line || prev.end_line || prev.loc?.start_line || prev.start_line || 0);
          const curStart = Number(cur.loc?.start_line || cur.start_line || 0);
          const adjacent = prevEnd + 1 >= curStart;
          const overlap  = prevEnd >= curStart;
          if (adjacent || overlap) {
            const mergedText = textOf(prev) + textOf(cur);
            const mergedTokens = estimateTokens({ text: mergedText }).inputTokens;
            if (mergedTokens <= merge_max_tokens) {
              manifest.metrics.merged_spans += 1;
            }
          }
        }
      }
