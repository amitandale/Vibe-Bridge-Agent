...
// Validate structure and recompute hash deterministically
  ...
  let globalTokens = 0;
  let globalFiles = 0;
  const seenContent = new Map(); // text -> { section, id } for cross-section pointers
  ...
  for (const name of ORDER){
    ...
    for (const s of nonDroppable){
      ...
      if (filesOver || tokensOver || secOver){
        if (seenContent.has(s.text)) {
          const ref = seenContent.get(s.text);
          placed.push({ id: s.id, ref, tokens: 0, pointer: true });
          continue;
        }
      ...
      if (!seenContent.has(s.text)) seenContent.set(s.text, { section: name, id: s.id });
    ...
    placed.push(...(typeof pointers !== 'undefined' ? pointers : []));
