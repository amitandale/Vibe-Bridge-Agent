...
for (const name of ORDER){
    ...
    const items = stableSort(rawItems.map(it => ({ ...it, path: toPosix(it?.path || it?.id || '') })) );
    ...
    for (const s of nonDroppable){
      ...
      placed.push({ id: s.id, path: toPosix(s.it.path || '') || undefined, text: s.text, tokens: s.tokens, __ord: { path: toPosix(s.it.path || ''), start: Number(s.it?.loc?.start_line ?? s.it?.start_line), sym: s.it?.symbol ?? '' } });
    ...
    const placedSorted = [...placed].sort((a,b)=>{
      const pa = a.__ord?.path || '';
      const pb = b.__ord?.path || '';
      if (pa !== pb) return pa < pb ? -1 : 1;
      const sa0 = a.__ord?.start;
      const sb0 = b.__ord?.start;
      const sa = Number.isFinite(sa0) ? sa0 : Number.POSITIVE_INFINITY;
      const sb = Number.isFinite(sb0) ? sb0 : Number.POSITIVE_INFINITY;
      if (sa !== sb) return sa - sb;
      const ya = a.__ord?.sym ?? '';
      const yb = b.__ord?.sym ?? '';
      const yAEmpty = ya === '';
      const yBEmpty = yb === '';
      if (yAEmpty !== yBEmpty) return yAEmpty ? 1 : -1;
      if (ya < yb) return -1;
    ...
    const placedOut = placedSorted.map(({__ord, ...rest}) => rest);
    manifest.sections.push({ name, items: placedOut });
