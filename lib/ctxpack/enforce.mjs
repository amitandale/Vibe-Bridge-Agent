...
/**
 * Full gate. Throws on failure in 'enforce' mode. Aggregates warnings in 'warn' mode.
 * Options:
...
export function gate(pack, {mode = process.env.CTXPACK_GATE || 'warn'} = {}) {
  ...
  try { validateObject(pack, { strictOrder: true }); }
  catch (e) { if (enforce) throw e; warnings.push(e.code || 'SCHEMA_INVALID'); }
  // deep invariants
  try { verifyPackHash(pack); }
  catch (e) { if (enforce) throw e; warnings.push(e.code || 'HASH_MISMATCH'); }
  try { enforceOrder(pack); }
  catch (e) { if (enforce) throw e; warnings.push(e.code || 'ORDER_VIOLATION'); }
  try { enforceBudgets(pack); }
  catch (e) { if (enforce) throw e; warnings.push(e.code || 'BUDGETS'); }
  try { enforceMustNever(pack); }
  catch (e) { if (enforce) throw e; warnings.push(e.code || 'NEVER_INCLUDE'); }
