...
/**
 * Grok (xAI) adapter. Treated as OpenAI-compatible chat completions for tests.
 * @param {{ model:string, apiKey?:string, baseUrl?:string, fetchImpl?:Function, timeoutMs?:number }} opts
 */
export function createGrok(opts = {}){
  const {
    model,
    apiKey = process.env.GROK_API_KEY || process.env.XAI_API_KEY || process.env.LLM_API_KEY || '',
    baseUrl = (process.env.GROK_BASE_URL || process.env.XAI_BASE_URL || 'https://api.x.ai').replace(/\/+$/,''),
