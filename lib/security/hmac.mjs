...
export function lookupKey(projectId, kid, { now = Date.now(), grace_s = DEFAULT_GRACE_S } = {}){
  // BA-02 O2: provider-aware lookup without changing async surface
  if (__secretsProvider) {
    try {
      if (typeof __secretsProvider.listActiveForProject === 'function') {
        const arr = __secretsProvider.listActiveForProject(projectId) || [];
        const cur = Array.isArray(arr) ? (arr.find(r => r.kid === kid) || arr[0]) : null;
        if (cur && (cur.value || cur.key)) {
          const val = cur.value || cur.key;
          return { key: val, kid: cur.kid || kid, source: 'current' };
        }
      }
      if (typeof __secretsProvider.getByKid === 'function') {
        const g = __secretsProvider.getByKid(kid);
        if (g && (g.projectId === projectId || g.project_id === projectId)) {
          const val = g.value || g.key;
          if (val) return { key: val, kid, source: 'current' };
        }
      }
    } catch {}
  }
