...
export async function verify({ projectId, kid, signatureHex, rawBody }) {
  ...
  for (const key of candidateKeys) {
    const mac = createHmac('sha256', Buffer.from(key.value, 'utf8')).update(body).digest('hex');
    if (ctEqual(mac, sig)) {
...
export async function sign({ keyValue, rawBody }) {
  const body = rawBody || Buffer.alloc(0);
  return createHmac('sha256', Buffer.from(keyValue, 'utf8')).update(body).digest('hex');
}
