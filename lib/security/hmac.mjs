...
/**
 * Testing/legacy hook for seeding. No side effects in overlay 1.
 * Accepts either (projectId, kid, key) or an options object.
 * Returns a normalized record useful for tests.
 */
export async function _seed(a, b, c) {
  const opts = (typeof a === 'object' && a) ? a : { projectId: a, kid: b, key: c };
  return {
...
/**
 * Lookup an active secret by projectId and kid using an injected provider.
 * Returns the SecretRecord or null.
 */
export async function lookupKey({ projectId, kid, getActiveSecretsByProject }) {
  if (!projectId || !kid || typeof getActiveSecretsByProject !== 'function') return null;
  const records = await getActiveSecretsByProject(projectId);
  const arr = Array.isArray(records) ? records : [];
  const k = String(kid);
  for (const r of arr) {
    if (Number(r?.active) === 1 && String(r?.kid) == k) return r;
  }
  return null;
}
