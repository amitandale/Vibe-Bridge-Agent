...
export function timingSafeEqualStr(a,b){ const A=Buffer.from(String(a)); const B=Buffer.from(String(b)); if (A.length!==B.length) return false; return require('crypto').timingSafeEqual(A,B); }


export function sign(rawBuf, key){ return 'sha256=' + require('crypto').createHmac('sha256', key).update(rawBuf).digest('hex'); }
