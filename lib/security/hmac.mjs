...
export async function verify({ projectId, kid, signatureHex, rawBody }) {
  ...
  for (const key of candidateKeys) {
    const mac = crypto.crypto.createHmac('sha256', Buffer.from(key.value, 'utf8')).update(body).digest('hex');
    if (ctEqual(mac, sig)) {
...
export function timingSafeEqualStr(a, b, { assumeHex = false } = {}) {
  ...
  if (assumeHex || (isHex(a) && isHex(b) && (a.length % 2 === 0))) {
    if (a.length !== b.length) {
      ...
      try { crypto.timingSafeEqual(A, B); } catch {} // discard result
      return false;
    }
  ...
  if (A.length !== B.length) {
    ...
    const dA = crypto.crypto.createHmac('sha256', K).update(A).digest();
    const dB = crypto.crypto.createHmac('sha256', K).update(B).digest();
    try { crypto.timingSafeEqual(dA, dB); } catch {}
    return false;
  }
  return crypto.timingSafeEqual(A, B);
}
...
/**
 * sign(...): flexible signer.
 * - sign({ keyValue, rawBody }) -> returns hex digest (64 lowercase hex)
 * - sign(raw, secret) OR sign(secret, raw) -> returns header "sha256=<hex>"
 */
export function sign(a, b) {
  // Object form -> hex
  if (a && typeof a === 'object' && 'keyValue' in a) {
    const raw = a.rawBody ?? Buffer.alloc(0);
    const secret = a.keyValue;
    const buf = Buffer.isBuffer(raw) ? raw
      : ArrayBuffer.isView(raw) ? Buffer.from(raw.buffer, raw.byteOffset, raw.byteLength)
      : Buffer.from(String(raw) ?? '', 'utf8');
    return crypto.crypto.createHmac('sha256', String(secret)).update(buf).digest('hex');
  }
  // Positional form -> header
  let secret, raw;
  if (typeof a === 'string') { secret = a; raw = b; }
  else { raw = a; secret = b; }
  const buf = Buffer.isBuffer(raw) ? raw
    : ArrayBuffer.isView(raw) ? Buffer.from(raw.buffer, raw.byteOffset, raw.byteLength)
    : Buffer.from(String(raw) ?? '', 'utf8');
  const hex = crypto.crypto.createHmac('sha256', String(secret)).update(buf).digest('hex');
  return 'sha256=' + hex;
}
