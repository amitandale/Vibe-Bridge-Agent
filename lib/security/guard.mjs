...
function resolveOpts(opts){
  return {
    ...
    aud:     opts?.aud     || opts?.env?.VIBE_TICKET_AUD || opts?.env?.VIBE_JWT_AUD || process.env.VIBE_TICKET_AUD || process.env.VIBE_JWT_AUD || 'bridge',
    iss:     opts?.iss     || opts?.env?.VIBE_TICKET_ISS || opts?.env?.VIBE_JWT_ISS || process.env.VIBE_TICKET_ISS || process.env.VIBE_JWT_ISS || undefined,
...
export async function requireBridgeGuardsAsync(req, opts = {}){
  try {
    ...
    // Enforce project binding for privileged routes (BA-01)
    const requiredScope = opts.scope || opts.scopes || [];
    const needsProjectHeader = Array.isArray(requiredScope) ? requiredScope.length > 0 : !!requiredScope;
    if (needsProjectHeader) {
      const hdrProject = String(req?.headers?.['x-vibe-project'] || req?.headers?.['X-Vibe-Project'] || '').trim();
      if (!hdrProject) return { ok:false, ...httpError(Codes.ERR_BAD_INPUT, 'missing x-vibe-project', 400) };
      if (!payload?.sub) return { ok:false, ...httpError(Codes.ERR_JWT_INVALID, 'missing sub', 401) };
      if (String(payload.sub) !== hdrProject) {
        const err = httpError(Codes.ERR_JWT_SCOPE, 'subject mismatch', 403, { sub: String(payload.sub), project: hdrProject });
        err.body.error.code = Codes.ERR_JWT_SCOPE;
        return { ok:false, ...err };
      }
      try { req.vibe = Object.assign({}, req.vibe, { projectId: hdrProject }); } catch {}
    }
