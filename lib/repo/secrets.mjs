...
/** Set or rotate an HMAC key for project. */
export async function setHmacKey({ projectId, kid, key }){
  ...
  if (db.hmac[projectId].byKid[kid] && db.hmac[projectId].byKid[kid] !== key){
    // allow rotation: overwrite value
  }
...
export async function setActiveHmacKid({ projectId, kid }){
  ...
  db.hmac[projectId].activeKid = kid;
  await writeStore(db);
  return { projectId, kid };
}
