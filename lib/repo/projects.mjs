...
export function validateOwner(owner){
  ...
  // GitHub owner: letters, digits, hyphen. Cannot start/end with hyphen. 1..39 chars.
...
export function validateRepo(repo){
  const s = String(repo||"");
  // GitHub repo: letters, digits, hyphen, underscore, dot. 1..100 chars. No space.
  if (s.length < 1 || s.length > 100) return false;
  if (!/^[A-Za-z0-9._-]+$/.test(s)) return false;
  return true;
}
export function setRepoBinding(id, { owner, repo }){
  if (!validateOwner(owner) || !validateRepo(repo)) throw new Error('INVALID_OWNER_OR_REPO');
  return upsert({ id, name: id, repo_owner: owner, repo_name: repo, disabled: 0 });
}
