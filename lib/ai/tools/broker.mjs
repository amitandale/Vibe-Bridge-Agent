...
/** Expand a glob within projectRoot and return project-relative paths */
...
export async function runNodeTestPattern({ projectRoot, pattern, timeoutMs=60000 }) {
  // Expand pattern into explicit files for node --test
  const files = await expandGlob(projectRoot, pattern);
  if (!files.length) {
    // No matches -> no-op success
...
/** createToolBroker matches legacy tests/imports */
