...
async function getPlannerPack({ projectRoot }) {
  // Dynamic import to avoid hard dependency
  let planPR;
  try {
    ({ planPR } = await import('../../planner/index.mjs'));
  } catch {
    return null;
  }
  ...
  if (!diff && diffPath) {
    try { diff = await fs.readFile(diffPath, 'utf8'); } catch {}
  }
...
async function ensureCtxpackGate({ projectRoot, onLog }) {
  ...
  let pack = null;
  if (plannerMode !== 'off') {
    pack = await getPlannerPack({ projectRoot });
  }
  if (!pack) {
    // Fallback minimal pack; pipelines will enrich in follow-ups.
    pack = buildPack({
    projectId: process.env.PROJECT_ID || 'unknown',
    pr: { id: process.env.PR_ID || 'unknown', branch: process.env.GIT_BRANCH || 'work', commit_sha: process.env.GIT_COMMIT || 'deadbee' },
    mode: process.env.CTXPACK_MODE || 'PR',
  ...
  try {
    gate(pack, { mode });
    const minfo = { mode, plannerMode, must: Array.isArray(pack.must_include)?pack.must_include.length:undefined };
    if (plannerMode === 'enforce' && (!Array.isArray(pack.must_include) || pack.must_include.length === 0)) {
      const err = Object.assign(new Error('Planner essential set missing'), { code:'MUST_INCLUDE_EMPTY' });
      throw err;
    }
    if (plannerMode === 'warn' && (!Array.isArray(pack.must_include) || pack.must_include.length === 0)) {
      onLog?.({ level:'warn', message:'planner produced empty must_include', meta: minfo });
    } else {
      onLog?.({ level:'info', message:'ctxpack gate ok', meta: minfo });
    }
  } catch (e) {
