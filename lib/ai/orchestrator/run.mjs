...
async function ensureCtxpackGate({ onLog } = {}) {
  const mode = process.env.CTXPACK_GATE || 'off';
  if (mode === 'off') return;

  // Optional planner preflight
  const plannerMode = process.env.CTXPACK_PLANNER || 'off';
  let pack = null;

  if (plannerMode !== 'off') {
    try {
      pack = await (async function getPlannerPack() {
        let planPR;
        try { ({ planPR } = await import('../../planner/index.mjs')); } catch { return null; }
        const fs = await import('node:fs/promises');
        const diffPath = process.env.DIFF_PATH;
        let diff = process.env.DIFF_TEXT || process.env.DIFF_PATCH || '';
        if (!diff && diffPath) { try { diff = await fs.readFile(diffPath, 'utf8'); } catch {} }
      ...
      onLog?.({ level:'warn', message:'planner pack building failed', meta:{ message: e.message } });
      pack = null;
    }
  }

  if (!pack) {
    // Fallback minimal pack; pipelines will enrich later.
    pack = buildPack({
      projectId: process.env.PROJECT_ID || 'unknown',
      pr: { id: process.env.PR_ID || 'unknown', branch: process.env.GIT_BRANCH || 'work', commit_sha: process.env.GIT_COMMIT || 'deadbee' },
      mode: process.env.CTXPACK_MODE || 'PR',
  ...
  try {
    gate(pack, { mode });
    // Pre-invoke assembly rollout: warn-first, hard-fail on non-droppable
    try {
      const assemblyMode = process.env.CTXPACK_ASSEMBLY || 'warn';
      const model = process.env.CTXPACK_MODEL || 'gpt-xyz';
      const outPath = process.env.CTXPACK_MANIFEST_OUT || 'assets/examples/ctxpack/manifest.latest.json';
      const { assembleAndPersist } = await import('../../ctxpack/integration.mjs');
      await assembleAndPersist(pack, { model, outPath, mode: assemblyMode, onLog });
    } catch (e) {
      onLog?.({ level:'error', message:'ctxpack assemble pre-invoke failed', meta:{ code:e.code } });
      throw e;
    }
    const mustLen = Array.isArray(pack.must_include) ? pack.must_include.length : 0;
    const meta = { mode, plannerMode, must: mustLen };
    if (plannerMode === 'enforce' && mustLen === 0) {
      const err = Object.assign(new Error('Planner essential set missing'), { code:'MUST_INCLUDE_EMPTY' });
      throw err;
    }
    if (plannerMode === 'warn' && mustLen === 0) {
      onLog?.({ level:'warn', message:'planner produced empty must_include', meta });
    } else {
      onLog?.({ level:'info', message:'ctxpack gate ok', meta });
    }
  } catch (e) {
