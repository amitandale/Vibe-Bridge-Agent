...
function stem(fileBase) {
  const b = String(fileBase || '');
...
function parseTrivial(stderr = '') {
  // Node's ESM error messages vary slightly by version; support common variants.
...
async function tryAutoFix({ projectRoot, changedFiles = [], classified }) {
  ...
  if (classified.kind === 'missing-export') {
    ...
    // 2) Fallback: search repo for definition
    ...
    if (def) {
      ...
      if (!alreadyExported) {
        ...
        const rel = def.replace(projectRoot.replace(/\/g,'/'), '').replace(/^\//, '');
        return { applied: true, kind: 'missing-export', file: rel, symbol: name };
      }
    }
  }
  // Potential extensions: handle 'import-not-found' by fixing import path.
...
export async function selfReview({
  ...
  changedFiles = [],
  profile = process.env.PROFILE || 'longrun',
  timeoutMs = 60000
} = {}) {
  if (profile !== 'longrun') {
