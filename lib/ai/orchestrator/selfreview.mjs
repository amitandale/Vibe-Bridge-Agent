...
function classifyFailure(stderrOrStdout) {
  ...
  // Node ESM: "The requested module '../lib/foo.mjs' does not provide an export named 'bar'"
  const m = s.match(/module ['"](.+?\.mjs)['"] does not provide an export named ['"](.+?)['"]/);
...
async function applyTrivialFix({ projectRoot, classified }) {
  ...
  // Normalize relative import to absolute path
