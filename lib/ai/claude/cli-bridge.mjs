    ...
    for (const line of lines) {
      ...
      if (obj && obj.type === 'tool_call') {
        Promise.resolve(onToolCall(obj.data))
          .then((result) => {
            proc.stdin.write(JSON.stringify({ type: 'tool_result', id: obj.id, result }) + '
');
          })
          .catch((err) => {
            proc.stdin.write(JSON.stringify({ type: 'tool_error', id: obj.id, error: String((err && err.message) || err) }) + '
');
          });
      }
  });
