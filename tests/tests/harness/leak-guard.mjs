...
function rel(p){ try { return String(p).replace(process.cwd().replace(/\\/g,'/') + '/', '').replace(/\\/g,'/'); } catch { return String(p);} }
...
function logLeakSummary(prefix){
  ...
  for (const [_, m] of REG.entries()) console.error(` - pending ${m.type} scheduled at ${m.site} @ ${m.at}`);
